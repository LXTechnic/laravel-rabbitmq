...
class RabbitMQQueue extends Queue implements QueueContract
{
    ...
    public function pushRaw($payload, $queueName = null, array $options = [])
    {
        try {
             ...
             * @var AmqpQueue $queue
             */
            [$queue, $topic, $routingKey] = $this->declareEverything($queueName);
    ...
    protected function declareEverything(string $queueName = null): array
    {
        ...
        if ($this->queueOptions['bindings'] ?? null) {
            foreach ($this->queueOptions['bindings'] as $binding) {
                $this->context->bind(new AmqpBind($queue, $topic, $binding));
            }
        } elseif ($this->queueOptions['bind']) {
            $this->context->bind(new AmqpBind($queue, $topic, $queue->getQueueName()));
        }

        $routingKey = $this->exchangeOptions['routing'] ?? $queueName;

        return [$queue, $topic, $routingKey];
    }
